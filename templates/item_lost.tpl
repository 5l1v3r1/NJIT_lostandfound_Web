...
{include file="config.tpl"}
	...
	<script type="text/javascript">
		...
		{foreach from=$founds item=found}mapdatas.push(["{$found['mapdata']}",{$found['id']},"{$found['what']}"]);{/foreach}
	</script>
	...
	<div id="main">
		<div class="container">
			<div class="contentLeft">
				<div id="accordion">
					...
					<div>
						...
						<tr><th>物品名称</th><td>{$item['what']}</td><td rowspan="5" class="imageCol"><img src="{$item['imageurl']|default:"images/noitem.png"}" id="image" class="image-fixed"></img></td></tr>
						<tr><th>丢失日期</th><td>{$item['when']|date_format:"%Y-%m-%d"}</td></tr>
						<tr><th>丢失地点</th><td>{if $item['where']!="#"}<a href="search.php?where={$item['where']}" class="where">{$item['where']}</a>{else}未注明{/if}</td></tr>
			...
			<div class="contentRight">
				<h3>相关招领信息</h3>
				<ul>
					{foreach from=$founds item=found}
						<li><a href="found.php?id={$found['id']}">{$found['what']}</a></li>
					{foreachelse}
						<li>没有相关招领启事</li>
					{/foreach}
				</ul>
