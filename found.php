...
switch($action){
	...
	case 'update':
		...
		header("Location: found.php?id=$_SESSION[id]");
	...
	case 'mod':
		...
		$sql = $db->prepare("SELECT `id`,`uid`,`gid`,`what`,`when`,`where`,`detail`,`tag`,`imageurl`,`result`,
			...
			FROM Items WHERE type=1 AND id=:id");
		$sql->bindParam(":id", $clean['id'], PDO::PARAM_INT);
		$sql->execute();
		...
		if($action=="del" || $_POST['del']){
			...
			$db->exec("DELETE FROM Items WHERE type=1 AND id=$clean[id]");
			...
			$smarty->display("item_found_edit.tpl");
		}
	...
	default:
		if(isset($_GET['id'])){
			...
			$sql = "SELECT `id`,`what`,ASTEXT(`mapdata`) as `mapdata`
				FROM Items
				WHERE `type`=0  AND `when`<='$item[when]' AND (";
			
			if($item['hasmap']){
				$sql .= "(!ISNULL(mapdata)
					AND CONTAINS(mapdata, GeomFromText('$item[mapdata]'))) ";
			}else{
				$sql .= "1 ";
			}
			if($item['tag']){
			...
			$result = $db->query($sql);
			while($lost = $result->fetch(PDO::FETCH_ASSOC)){
				array_push($losts, $lost);
			}
			...
			$smarty->assign("item", $item);
			$smarty->assign("losts", $losts);
			$smarty->assign("comments", $comments);
			$smarty->assign("pagenav", $pagenav);
			$smarty->display("item_found.tpl");
	
		}else{
		
			$result = $db->query("SELECT count(*) FROM Items WHERE type=1");
